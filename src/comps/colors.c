...
void comps_colors_render() {
	...
	for (int c = 0; c < 64; c++) {
		...
		Raquet_SetDrawColor(RQ_PAL(c), 255);
		SDL_RenderFillRect(Raquet_Renderer, &(SDL_FRect) { xoff, yoff, swatch_size, swatch_size });
		if (swatch_size >= 24) {
			ascii_color_set(RQ_PAL(c) ^ 0xffffff0f);
			unsigned char hex[3];
	...
	Raquet_SetDrawColor((palette_current_color_id < 0x20) ? RQ_PAL(0x30) : RQ_PAL(0x26), 255);
